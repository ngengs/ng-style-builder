...
const write = require('../lib/files').write;
...
let pkgOutputReserve = {
  description: 'Style builder generated by ' + pkg.name,
...
const main = async (data) => {
  ...
  const destination = data.package;
  ...
  if (!emptyDir) {
    ...
    const canContinue = await prompt.prompt({
      type: 'confirm',
      name: 'value',
      message: `Directory with name '${data.package}' already exist and not empty, continue?`,
      default: true
    });
  ...
  let resultCopyJs = await templates.copyRecursive('js', destination);
  if (!resultCopyJs) {
  ...
  logger.log(`Preparing style builder with "${data.style}"...`);
  ...
  if (data.style === 'less') {
    ...
    let result = await templates.copyRecursive('less', destination);
    if (!result) {
  ...
  let pkgOut = {};
  // noinspection JSUnresolvedVariable
  pkgOut.name = data.package;
  pkgOut.title = data.title;
  pkgOut.version = data.version;
